...
export const Projects: React.FC<ProjectsProps> = ({ projects, onSetActive, onCreate }) => {
  // Modal & Form State
                ...
                <div className="grid grid-cols-4 gap-4 flex-1 max-w-2xl bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                  ...
                  <div className="space-y-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className={`text-sm font-semibold ${isActive ? 'text-indigo-600' : 'text-slate-400'}`}>
                        {project.status}
                      </p>
                    </div>
                    {!isActive && (
                       <button 
                         ...
                         className="mt-2 text-xs font-medium px-3 py-1.5 rounded-lg border border-indigo-200 bg-white text-indigo-600 hover:bg-indigo-50 transition-colors shadow-sm w-fit"
                       >
                         Set Active
                       </button>
                    )}
