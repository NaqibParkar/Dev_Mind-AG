...
import { getProjectAnalyticsData } from '../constants';
import { TimeGranularity, MockDataPoint, Project } from '../types';
import { 
...
export const Analytics: React.FC<AnalyticsProps> = ({ activeProject }) => {
  ...
  // Dynamic Data Selection based on granularity and active project
  const currentData = useMemo(() => {
    return getProjectAnalyticsData(activeProject ? activeProject.id : null, granularity);
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${
              metrics.focus ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-400'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${
              metrics.avgFocus ? 'bg-purple-50 border-purple-200 text-purple-700' : 'bg-white border-slate-200 text-slate-400'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${
              metrics.workload ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-white border-slate-200 text-slate-400'
            }`}
          >
           ...
           <button 
            ...
            className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${
              metrics.avgWorkload ? 'bg-teal-50 border-teal-200 text-teal-700' : 'bg-white border-slate-200 text-slate-400'
            }`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.focus && (
          <Card className="border-t-4 border-t-indigo-500">
             ...
             <h3 className="text-2xl font-bold text-slate-700 mt-1">
               {currentData[currentData.length -1].focus} <span className="text-sm font-normal text-slate-400">/100</span>
             </h3>
        ...
        {metrics.workload && (
          <Card className="border-t-4 border-t-amber-500">
             ...
             <h3 className="text-2xl font-bold text-slate-700 mt-1">
               {currentData[currentData.length -1].workload} <span className="text-sm font-normal text-slate-400">/100</span>
             </h3>
