...
from app.db.models import User, Project, ActivityLog, Settings, JournalEntry
...
def main(confirm_wipe: bool = False):
    if not confirm_wipe:
        raise SystemExit(
            "Refusing to drop database tables without --confirm-wipe."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--confirm-wipe",
        action="store_true",
        help="Acknowledge that all configured database tables will be dropped.",
    )
    args = parser.parse_args()
    main(confirm_wipe=args.confirm_wipe)
